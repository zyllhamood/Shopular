...
export default function Orders() {
    ...
    useEffect(() => {
        if (orders === null && userAuth !== null) {
            fetch(`https://api.igstore.io/orders/`, {
                headers: {
    ...
    const handleSearch = (word) => {
        if (word.length >= 1) {
            let filterOrders = orders.filter(prd => (
                prd.product_name !== null && prd.product_name.toLowerCase().includes(word.toLowerCase()) ||
            ...
            >
                ...
                <TableContainer width={'98%'} overflowY={'scroll'}>
                    <Table variant='simple'>
                        <Thead>
                            <Tr>
                                <Th color={'#fff'}>Code</Th>
                                <Th color={'#fff'}>Username</Th>
                                <Th color={'#fff'}>Status</Th>
                                <Th color={'#fff'}>Product</Th>
                                <Th color={'#fff'}>Review</Th>
                        ...
                        <Tbody >
                            {filterdSearch !== null ? filterdSearch.filter((prd) => status === 'confirmed' ? prd.status === status : true).map((item) => (
                                <Tr _hover={{ bgColor: '#26374d' }} cursor={'pointer'} onClick={() => handleRowClick(item)} key={item.id}>
                                ...
                                <Tr>
                                    ...
                                    <Td><Skeleton width={'100px'} height={'20px'} /></Td>
                                    <Td><Skeleton width={'100px'} height={'20px'} /></Td>
                                    <Td><Skeleton width={'100px'} height={'40px'} /></Td>
                                </Tr>
                            )}
            ...
            <ModalContent bgColor={bk1}>
                ...
                <ModalBody>
                    <Flex flexDir={'column'}>
                        ...
                        <Text>Actually Paid : {item.actually_paid === null ? 0 : item.actually_paid}</Text>
